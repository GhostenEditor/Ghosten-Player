                                ...
                                return SlidableSettingItem(
                                  actions: [
                                    if (item.status == ScheduleTaskStatus.running)
                                      TVIconButton(onPressed: () => Api.scheduleTaskPauseById(item.id), icon: const Icon(Icons.pause_rounded)),
                                    if (item.status == ScheduleTaskStatus.paused)
                                      TVIconButton(onPressed: () => Api.scheduleTaskResumeById(item.id), icon: const Icon(Icons.pause_rounded)),
                                    TVIconButton(onPressed: () => Api.scheduleTaskDeleteById(item.id), icon: const Icon(Icons.delete_outline)),
                                  ],
                                  title: switch (item.type) {
                                    ScheduleTaskType.syncLibrary => Text(AppLocalizations.of(context)!.scheduleTaskSyncTitle(item.status.name)),
                                    ScheduleTaskType.scrapeLibrary => Text(AppLocalizations.of(context)!.scheduleTaskScrapeTitle(item.status.name)),
                                  },
                        ...
                        return SlidableSettingItem(
                          ...
                          actions: [
                            TVIconButton(
                              ...
                              onPressed: () => showNotification(context, Api.libraryRefreshById(item.id, true), showSuccess: false),
                            ),
                            TVIconButton(
                              icon: const Icon(Icons.cloud_sync_rounded),
                              onPressed: () => showNotification(context, Api.libraryRefreshById(item.id, false), showSuccess: false),
                            ),
                            TVIconButton(
